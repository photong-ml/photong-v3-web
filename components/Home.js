...
export default function Home() {
  ...
  const generateAudio = React.useCallback(async () => {
    ...
    const res = await fetch(`${config.API_URL}/infer?seed_img=${seedImg}&seed=${
                                seed}&alpha=${alphas[nalphaIndex]}`,
                            {
                              method : 'POST',
                              body : formData,
                            });
  ...
  const extendAudio = React.useCallback(async () => {
    ...
    // skipcq: JS-0002
    console.log(`Extending audio prompt ${prompt} seed ${nseed} alpha ${
        alphas[nalphaIndex]}`);

    const res = await fetch(
        `${config.API_URL}/infer/with-prompt?seed_img=${seedImg}&seed=${
            nseed}&prompt=${prompt}&alpha=${alphas[nalphaIndex]}`,
        {
          method : 'POST',
        },
    );
  ...
  const clearAudios = React.useCallback(() => { setAudios([]); }, []);

  React.useEffect(() => {
    if (uploaded && audios.length > 0 && audios.length < MAX_FILES)
      extendAudio();
  }, [ audios ]);
